...
interface PlatformStats {
  users: number
  suppliers: Array<{ status: string; _count: { _all: number } }>
  rfqs: Array<{ status: string; _count: { _all: number } }>
  quotations: number
  awards: number
}
...
export default function AdminDashboard() {
  ...
  const getCount = (arr: PlatformStats['suppliers'] | undefined, key: string) =>
    arr?.find((s) => s.status === key)?._count?._all ?? 0

  return (
    <div className="min-h-screen bg-gray-50">
      <AdminNav title="Platform Overview" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {['ACTIVE', 'PENDING', 'SUSPENDED', 'DEBARRED'].map((status) => {
              ...
              const total = loading ? 1 : stats?.suppliers?.reduce((s, c) => s + c._count._all, 0) || 1
              const pct = Math.round((count / total) * 100)
              const colors: Record<string, string> = {
                ACTIVE: 'bg-green-500', PENDING: 'bg-yellow-500',
                SUSPENDED: 'bg-orange-500', DEBARRED: 'bg-red-500',
              }
              return (
            ...
            <div className="space-y-3">
              {[
                ...
                <Link key={a.href} href={a.href} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors border border-gray-100">
                  <div className="w-9 h-9 bg-red-100 text-red-700 rounded-lg flex items-center justify-center flex-shrink-0">
                    <a.icon size={18} />
                  </div>
                  <div>
