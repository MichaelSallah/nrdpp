...
export default function AdminReportsPage() {
  ...
  if (loading) return <div><TopBar title="Platform Reports" /><div className="p-8 text-center text-gray-400">Loading...</div></div>
  ...
  const awardedRfqs = stats?.rfqs.find(r => r.status === 'AWARDED')?._count ?? 0
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="h-2 bg-gray-100 rounded-full">
                          <div className="h-full bg-green-600 rounded-full" style={{ width: `${(s._count.quotations / max) * 100}%` }} />
                        </div>
                      </div>
                    </div>
                  )
              ...
              <div className="divide-y divide-gray-50">
                {pricing.recentAwards.map((a) => (
                  <div key={a.id} className="py-3 flex justify-between gap-4">
                    ...
                    <p className="text-sm font-semibold text-gray-700 shrink-0">{formatCurrency(a.quotation?.totalAmount ?? 0)}</p>
                  </div>
                ))}
              </div>
        ...
        {pendingSuppliers > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center justify-between">
            ...
            <a href="/admin/suppliers" className="text-sm font-semibold text-yellow-800 hover:underline">Review →</a>
          </div>
        )}
      </div>
    </div>
